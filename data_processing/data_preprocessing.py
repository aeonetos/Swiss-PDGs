...
def get_all_test_id(grid_type, save=True):
    """
    This function gets all the test IDs
    :return: list, all the test IDs
    """
    if grid_type == 'MV':
        path_base = "grids/geojson_data/"+grid_type + "/"
...
def classify_grids(region_geo, grid_type):
    ...
    if grid_type == 'LV':
        path_base = "grids/geojson_data/LV/"
        list_folders = os.listdir(path_base)
        dict_test_id_folder = {}
        for j in range(len(list_folders)):
            path = path_base + list_folders[j] + "/"
    ...
    for i in test_id_lists:
        ...
        # read the nodes
        if grid_type == 'MV':
            path = "grids/geojson_data/"+grid_type + "/"
        else:
            path = "grids/geojson_data/"+grid_type + "/" + dict_test_id_folder[i] + "/"
        file_name = i + '_nodes'
        nodes_gpd = gpd.read_file(path + file_name)
    ...
    # save the dictionary
    if grid_type == 'MV':
        df_transformer[['grid_id', 'region_name']].to_csv('table_grid_canton_' + grid_type + '.csv', index=False)
    ...
    if grid_type == 'LV':
        df_transformer[['grid_id', 'region_name']].to_csv('table_grid_municipality_' + grid_type + '.csv', index=False)
...
def connect_mv_lv():
    ...
    mv_files = os.listdir('grids/geojson_data/MV/')
    ...
    for m in mv_ids:
        ...
        # read the nodes
        nodes_gpd = gpd.read_file('grids/geojson_data/MV/' + m + '_nodes')
...
if __name__ == '__main__':
    ...
    #canton_gpd = gpd.read_file('cantons_union.geojson')
    ...
    #canton_geo.to_file('canton_union.geojson', driver='GeoJSON')
    #
    #canton_boundary = gpd.read_file('canton_union.geojson')
    #
    ...
    #canton_boundary.to_csv('canton_coordinates_plot.csv', index=False)
    #
    ...
    #municipality_geo.to_file('municipality_boundary.geojson', driver='GeoJSON')
    
    #Update the dictionary of canton and municipality
    '''
    mv_test_list, lv_test_list = get_all_test_id('MV', save=True), get_all_test_id('LV', save=True)
    conton_geo= gpd.read_file('canton_union.geojson')
    classify_grids(conton_geo, 'MV')
    municipality_geo= gpd.read_file('municipality_boundary.geojson')
